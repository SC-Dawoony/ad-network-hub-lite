...
from ui.create_app.simple_mode.payload_preview.ui import render_payload_preview
from ui.create_app.simple_mode.payload_preview.logic import generate_preview_data
...
def render_new_create_app_ui():
    ...
    if android_package or ios_bundle_id:
        ...
        if selected_networks:
            ...
            # Generate preview data using logic module
            preview_data, has_errors = generate_preview_data(
                selected_networks,
                available_networks,
                store_info_ios=st.session_state.get("store_info_ios"),
                store_info_android=st.session_state.get("store_info_android"),
                ios_ad_unit_identifier=st.session_state.get("ios_ad_unit_identifier")
            )
            
            # Store preview_data in session state
            st.session_state.preview_data = preview_data
            
            # Display previews using UI module
            render_payload_preview(preview_data, has_errors)
